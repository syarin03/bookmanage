    ...
    while True:
        ...
        if sel == '1':
            ...
            if sel2 == '2':
                ...
                if sel6 == '1':
                    print(nowlogin.book)
                    sel7 = input("고유번호를 입력헤주세요\n> ")
                    for j in nowlogin.book:  # 사용자가 가지고 있는 책리스트의 첫번째부터 서치
                        for i in dic.values():
                            if i == dic[sel7] and i[:2] == j[
                                                           1:]:  # 001의 value가 입력한 키의 value와 같고, 해당 value의 0번,1번이 해당 nowlogin.book의 첫번째와 같을 때
                                rebook.append(j)
                ...
                if sel6 == '2':
                    ...
                    for j in dic.keys():
                        if dic[j][1].find(
                                set8) != -1:  # i = [작가, 책이름, 상태], nowlogin.book = [[작가, 책이름],[작가, 책이름],[작가, 책이름]...]
                            cnt1 = 0
                            print(dic[j][0:2])
            ...
            doname = input("기증하실 도서의 저자를 입력해주세요.\n")
    ...
    if sel == '1':
        ...
        while loop:
            ...
            if find_sel == '1':
                ...
                for i in userList:
                    if input("아이디를 입력해주세요\n> ") == i.user_id:
                        if input("전화번호를 입력해주세요\n> ") == i.phone:
